...
{
	// OPL3 carrier-operator index for 2-op channel ch (0..17):
	//   channels 0..8  -> first register set,  carriers op[9..17]
	//   channels 9..17 -> second register set, carriers op[27..35]
	int carrier = (ch < 9) ? (9 + ch) : (27 + (ch - 9));
	if (ch < 0 || carrier >= (int)MAXOPERATORS) return 0.0;
	op_type *o = &op[carrier];
